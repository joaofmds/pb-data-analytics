...
select 
	case 
		...
		when professional_status = 'bussinessman' then 'empresário(a)'
...
with 	
	classificacao_veiculos as (
		...
		select 	
			...
			end as "clasificação do veículo"
...
select
	"clasificação do veículo",
	count(*) "veículos visitados (#)"
from classificacao_veiculos
group by "clasificação do veículo"
...
select
	"idade do veículo",
	count(*)::float/(select count(*) from sales.funnel) "veículos visitados (%)",
	ordem
